...
const TimeSlotRow = React.memo(({ timeSlot, onTimeChange, onRemove }) => {
    const inputRef = useRef(null);

    const handleInputChange = useCallback((e) => {
        onTimeChange(timeSlot.id, e.target.value);
        ...
        <div className="group mb-4">
            <div className="flex items-center space-x-3">
                ...
                <input
                    ...
                    className="flex-1 px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 border-gray-200 focus:border-blue-500 focus:ring-blue-100"
                    autoComplete="off"
                    spellCheck="false"
                />
                <button
...
const TimeSlotsEditorModal = ({ isOpen, onClose, initialTimeSlots, onSave, isOnline }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            const slots = (initialTimeSlots || []).map((slot) => ({
                id: `slot-${nextIdRef.current++}`,
                value: slot || ""
            }));
            if (slots.length === 0) slots.push({ id: `slot-${nextIdRef.current++}`, value: "" });
            setTimeSlots(slots);
        }
    ...
    const handleSaveChanges = async () => {
        setIsSaving(true);
        try {
            const validSlots = timeSlots.map(slot => slot.value).filter(Boolean);
    ...
    const isSaveDisabled = !isOnline || isSaving || !hasValidSlots;
