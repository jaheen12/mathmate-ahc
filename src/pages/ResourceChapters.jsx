    ...
    return (
        <div className="p-2">
            <div className="flex justify-between items-center mb-4">
                <Link to="/resources" className="text-gray-600 hover:text-gray-800 p-2">
                    <IoArrowBack size={24} />
                </Link>
                {currentUser && (
            ...
            {loading ? (
                ...
                chapters.length > 0 ? (
                    <ul className="space-y-3">
                        {chapters.map(chapter => (
                            <li key={chapter.id} className="flex justify-between items-center p-4 bg-white rounded-lg shadow-md transition-shadow hover:shadow-lg">
                                <span onClick={() => navigate(`/resources/${categoryId}/${chapter.id}`)} className="cursor-pointer font-semibold text-lg text-gray-800 flex-grow">
                                    {chapter.name}
                                </span>
                                {currentUser && (
                                    <div className="flex items-center gap-2">
                    ...
                    <div className="text-center mt-10">
                        ...
                        <p className="text-gray-500 mt-2">Click "Add Chapter" to organize your resources.</p>
                    </div>
                )
            )}
        </div>
